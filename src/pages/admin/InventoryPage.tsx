...
export default function InventoryPage() {
  ...
  const productName = useMemo(() => {
    ...
    return (id: number) => m.get(id) ?? `Product #${id}`;
  }, [products]);
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="mb-5">
        <h1 className="font-display text-2xl font-bold text-zinc-900">Inventory</h1>
        <p className="text-sm text-zinc-500">Track stock levels and every movement.</p>
      </div>
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 md:grid-cols-3">
        <div className="flex items-center gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-100">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-400">Products tracked</p>
          </div>
        </div>
        <div className="flex items-center gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-100">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-400">Low stock (≤ 8)</p>
          </div>
        </div>
        <div className="col-span-2 flex items-center gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-100 md:col-span-1">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-400">Restocks logged</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-5">
        ...
        <div className="rounded-2xl bg-white shadow-sm ring-1 ring-zinc-100 xl:col-span-3">
          <h2 className="border-b border-zinc-100 px-5 py-4 font-display text-sm font-bold text-zinc-900">Stock levels</h2>
              ...
              <li key={p.id} className="flex items-center gap-3 px-5 py-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {p.stock <= 8 && <span className="rounded-full bg-amber-100 px-1.5 py-0.5 text-[9px] font-bold text-amber-700">LOW</span>}
                  </div>
                ...
                <span className="w-12 text-right font-display text-sm font-bold text-zinc-900">{p.stock}</span>
        ...
        <div className="rounded-2xl bg-white shadow-sm ring-1 ring-zinc-100 xl:col-span-2">
          <h2 className="border-b border-zinc-100 px-5 py-4 font-display text-sm font-bold text-zinc-900">Movement history</h2>
          <ul className="thin-scroll max-h-[560px] divide-y divide-zinc-50 overflow-y-auto">
            {logs.length === 0 && <li className="px-5 py-8 text-center text-xs text-zinc-400">No movements yet.</li>}
            {logs.map((l) => (
              <li key={l.id} className="flex items-center gap-3 px-5 py-2.5">
                ...
                <span className="w-14 text-right text-[10px] text-zinc-300">{timeAgo(l.created_at)}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <Modal open={!!target} onClose={() => setTarget(null)} title={`Adjust stock — ${target?.name ?? ''}`}>
        <div className="space-y-3">
          ...
          <div>
            <label className="mb-1 block text-[10px] font-bold uppercase text-zinc-400">Quantity</label>
          ...
          <div>
            <label className="mb-1 block text-[10px] font-bold uppercase text-zinc-400">Notes</label>
            <input value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="e.g. weekly supplier delivery" className="w-full rounded-xl border border-zinc-200 px-3.5 py-2.5 text-sm outline-none focus:border-brand-400" />
          </div>
